...
python3 -m verl.trainer.main_ppo_inference \
    ...
    +env.webshop.end_idx=128 \
    +env.save_trajectories=True \
    +env.trajectory_save_dir=${project_root}/output/inference/trajectories \
    trainer.critic_warmup=0 \
    trainer.logger=['console'] \
    trainer.project_name='verl_agent_webshop_inference' \
...
python3 ${SCRIPT_DIR}/merge_trajectories.py \
    ${project_root}/output/inference/trajectories \
