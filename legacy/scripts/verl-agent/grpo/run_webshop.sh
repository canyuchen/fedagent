...
# (Removed `export VLLM_ATTENTION_BACKEND=XFORMERS` - was forcing fallback to slower V0 engine.)
...
# Skip if parquet already exists (avoids race when multiple sweeps run concurrently writing the same file)
if [ ! -f $project_root/data/verl-agent/text/train.parquet ]; then
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.fedprox_mu=${FEDPROX_MU:-0.0} \
