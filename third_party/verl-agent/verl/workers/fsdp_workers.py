...
from verl.utils.checkpoint.fsdp_checkpoint_manager_fed import FSDPCheckpointManager, StateDictType
...
class ActorRolloutRefWorker(Worker):
    ...
    # ---------------------------------------------------------------------
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)  # add the decorator if you want to pull in batch
    def set_weights(self, state_dict: dict, which: str = "actor", strict: bool = True):
        """
        Set the FSDP state_dict for the specified role.
        """
        try:
            if which == "actor":
                assert hasattr(self, "actor_module_fsdp"), "Actor model has not been initialized yet"
                target_model = self.actor_module_fsdp
            elif which == "ref":
                assert hasattr(self, "ref_module_fsdp"), "Ref model has not been initialized yet"
                target_model = self.ref_module_fsdp
            else:
                raise ValueError(f"which must be 'actor' or 'ref', got {which}")
            
            print(f"Loading {which} weights...")
            print(f"State dict keys count: {len(state_dict)}")
            
            # Check the FSDP version and use the corresponding loading method
            from torch.distributed.fsdp import FullyShardedDataParallel as FSDP

            if isinstance(target_model, FSDP):
                # Use the FSDP-specific loading method
                from torch.distributed.fsdp import StateDictType, FullStateDictConfig

                # Temporarily switch to FULL_STATE_DICT mode
                ...
                # For non-FSDP models, use the standard method
                missing_keys, unexpected_keys = target_model.load_state_dict(
                    state_dict, strict=strict
                )

            # Report the loading result
            if missing_keys:
                if strict:
                    raise RuntimeError(f"Missing keys in {which} state_dict: {missing_keys}")
                else:
                    print(f"Warning: Missing keys in {which}: {missing_keys[:5]}...")
            
            if unexpected_keys:
                if strict:
                    raise RuntimeError(f"Unexpected keys in {which} state_dict: {unexpected_keys}")
                else:
                    print(f"Warning: Unexpected keys in {which}: {unexpected_keys[:5]}...")
            
            print(f"Successfully loaded {which} weights")
            ...
            "message": f"Successfully loaded {which} weights"
        }
            
        except Exception as e:
            print(f"Failed to set {which} weights: {str(e)}")
            if strict:
                raise e


    @register(dispatch_mode=Dispatch.ONE_TO_ALL)  # add the decorator if you want to pull in batch
    def get_weights(self, which: str = "actor"):
        """
        Use a safer parameter extraction method.
        """
        try:
            if which == "actor":
                target_model = self.actor_module_fsdp
            ...
            from torch.distributed.fsdp import StateDictType, FullStateDictConfig
            if isinstance(target_model, FSDP):
            # Option 1: use rank0_only=True so only rank0 gathers the full parameters
                if self.rank == 0:
                    with FSDP.state_dict_type(
                        target_model,
                        StateDictType.FULL_STATE_DICT,
                        ...
                        # Make sure the parameters are on CPU
                        cpu_state_dict = {}
                        for key, value in full_state_dict.items():
                            if isinstance(value, torch.Tensor):
                                cpu_state_dict[key] = value.cpu().detach()
                            else:
                                cpu_state_dict[key] = value

                        return cpu_state_dict
                else:
                    # Other ranks only need to participate in FSDP internal communication, then return an empty dict
                    with FSDP.state_dict_type(
                        target_model,
                        StateDictType.FULL_STATE_DICT,
                        FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
                    ):
                        # This participates in FSDP internal communication but does not return the full parameters
                        _ = target_model.state_dict()

                    return {}
            else:
                # Non-FSDP model: return directly
                return target_model.state_dict()

        except Exception as e:
            print(f"Failed to get {which} weights: {str(e)}")
            return {}
    # ---------------------------------------------------------------------
...
class CriticWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)  # add the decorator if you want to pull in batch
    def set_weights(self, state_dict: dict, which: str = "critic", strict: bool = True):
        """
        Set the FSDP state_dict for the specified role.
        """
        try:
            if which in ['critic']:
                    # Check the FSDP version and use the corresponding loading method
                target_model = self.critic_module
            else:
                raise ValueError(f"Unsupported role: {which}. Only 'critic' is supported.")
            ...
            if isinstance(target_model, FSDP):
                ...
                # Use the FSDP load_state_dict method directly
                missing_keys, unexpected_keys = target_model.load_state_dict(
                    state_dict, strict=strict
                )
            ...
            if strict:
                raise e
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)  # add the decorator if you want to pull in batch
    def get_weights(self, which: str = "critic") :
        """
        Return the current model weights (state_dict).
        If you only want a particular submodule, you can filter and return it yourself.
        """
        try:
            if which not in ['critic']:
                raise ValueError(f"Unsupported role: {which}. Only 'critic' is supported.")
            target_model = self.critic_module
            from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
            ...
            print(f"Failed to get {which} weights: {str(e)}")
        return {}
        # return self.critic_module.state_dict()     # the corresponding module such as critic/actor
