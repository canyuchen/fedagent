...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Allow saving only the model, but it must include the model
        assert "model" in checkpoint_contents, f"FSDPCheckpointManager must include 'model', got {checkpoint_contents}"
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        # Decide which contents to load based on checkpoint_contents
        if "model" in self.checkpoint_contents:
            ...
            if del_local_after_load:
                try:
                    ...
                    print(f"[rank-{self.rank}]: remove local model file after loading failed, exception {e} will be ignored")
            
            state_dict_cfg = ShardedStateDictConfig(offload_to_cpu=True if is_cuda_available else False)
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, None):
                self.model.load_state_dict(model_state_dict)
        
        if "optimizer" in self.checkpoint_contents and self.optimizer is not None:
            ...
            if del_local_after_load:
                try:
                    ...
                    print(f"[rank-{self.rank}]: remove local optimizer file after loading failed, exception {e} will be ignored")
        ...
        if "extra" in self.checkpoint_contents:
            ...
            if del_local_after_load:
                try:
                    ...
                    print(f"[rank-{self.rank}]: remove local extra state file after loading failed, exception {e} will be ignored")
            ...
            if self.lr_scheduler is not None and "lr_scheduler" in extra_state_dict:
                lr_scheduler_state_dict = extra_state_dict["lr_scheduler"]
                self.lr_scheduler.load_state_dict(lr_scheduler_state_dict)

    def load_params(self, params: dict, strict: bool = False):
        """
        ...
        # Use FSDP's load_state_dict to handle sharded state
        state_dict_cfg = ShardedStateDictConfig(offload_to_cpu=True if is_cuda_available else False)
        optim_cfg = ShardedOptimStateDictConfig(offload_to_cpu=True if is_cuda_available else False)
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                # Decide which contents to save based on checkpoint_contents
                if "model" in self.checkpoint_contents:
