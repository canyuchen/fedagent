...
class RayPPOTrainer:
    ...
    def _save_checkpoint(self):
        # Prefer trainer.save_dir; fall back to default_local_dir if it is not set
        save_dir = getattr(self.config.trainer, 'save_dir', None)
        if save_dir is not None:
            local_global_step_folder = os.path.join(save_dir, f"global_step_{self.global_steps}")
        else:
        ...
        # Use the same directory as the checkpoint save path
        latest_checkpoint_dir = save_dir if save_dir is not None else self.config.trainer.default_local_dir
        local_latest_checkpointed_iteration = os.path.join(latest_checkpoint_dir, "latest_checkpointed_iteration.txt")
    ...
    def fit(self):
        ...
        from verl.utils.tracking_fed import Tracking
        # +f"_cl_{self.client_id}_rd_{self.round_num}"
        logger = Tracking(
